...
exports.BookingUseCases = void 0;
const crypto_1 = require("crypto");
const entities_1 = require("../../domain/entities");
class BookingUseCases {
    constructor(bookingsRepository, spacesRepository, validationService, logger) {
    ...
    async createBooking(dto) {
        ...
        const validation = await this.validationService.validateNewBooking(dto.spaceId, dto.clientEmail, date, dto.startTime, dto.endTime);
        if (!validation.isValid) {
            const error = new Error(`Validation error: ${validation.errors.join(", ")}`);
            this.logger.error("Error in booking validation", error, dto);
            throw error;
        }
        const booking = entities_1.Booking.createNew((0, crypto_1.randomUUID)(), dto.spaceId, dto.clientEmail, date, dto.startTime, dto.endTime);
    ...
    async updateBooking(dto) {
        ...
        if (dto.date || dto.startTime || dto.endTime) {
            const validation = await this.validationService.validateNewBooking(existingBooking.getSpaceId(), existingBooking.getClientEmail(), newDate, newStartTime, newEndTime);
            if (!validation.isValid) {
                throw new Error(`Validation error: ${validation.errors.join(", ")}`);
            }
        }
        const updatedBooking = entities_1.Booking.create(existingBooking.id, existingBooking.getSpaceId(), existingBooking.getClientEmail(), newDate, newStartTime, newEndTime, existingBooking.getStatus(), existingBooking.createdAt, new Date());
    ...
    async getBookingsBySpace(spaceId) {
...
exports.BookingUseCases = BookingUseCases;
